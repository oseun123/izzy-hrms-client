...
import { updateCompany } from '../../../../../../store/actions/preferencesActions';
...
function EditCompany() {
  ...
  const { data } = useGetSystemCompany(enabled, setEnabled, 'all');

  // Effect to set selected gender when data changes
  useEffect(() => {
    if (data && Object.keys(data).length) {
      const companys = data.payload?.companys || [];
      const selectcompany = companys.find((item) => item.id === parseInt(id));
      setSelected(selectcompany || null);
    }
  }, [data, id]);
  ...
  function updateCompanyCallback() {
    setLoading(true);
    updateCompany(dispatch, request, values).then(() => {
      setLoading(false);
    });
  }
  ...
  const { values, errors, handleChange, handleSubmit, clearForm } =
    useCustomForm(updateCompanyCallback, initValues, validateCreateCompany);
