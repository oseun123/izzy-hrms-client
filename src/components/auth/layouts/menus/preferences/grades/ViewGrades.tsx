...
import { deleteGrade } from '../../../../../../store/actions/preferencesActions';

import { useMediaQuery } from 'react-responsive';
import { grade_columns } from './../../../../../../util/tables';
import PreferencesHero from '../PreferencesHero';
import AminatedLayout from '../../../../../ui/AminatedLayout';
import NoCustomDataIcon from '../../../../../ui/NoCustomDataIcon';
import { useGetSystemGradePaginated } from '../../../../../../store/actions/preferencesHooksActionsType';
import { Grade, User } from '../../../../../../@types/api.types';
...
function ViewGrades() {
  ...
  const { data, isLoading } = useGetSystemGradePaginated(
    enabled,
    setEnabled,
    page,
    size,
  ...
  const delete_grade = useSelector(
    // @ts-ignore
    (state) => memoUserpermission(state, 'DELETE_GRADES'),
    shallowEqual,
  );
  const edit_grade = useSelector(
    // @ts-ignore
    (state) => memoUserpermission(state, 'EDIT_GRADES'),
    shallowEqual,
  );

  const isTabletOrMobile = useMediaQuery({ maxWidth: 1224 });
  const confirm_text = 'Are you sure you want to delete this grades?';
  const request = useAxiosPrivate();

  useEffect(() => {
    if (data && Object.keys(data).length) {
      setGrade(data?.payload?.grades);
    }
  }, [data]);
  ...
  function confirmAction(id: number) {
    deleteGrade(dispatch, request, { id }).then((res) => {
