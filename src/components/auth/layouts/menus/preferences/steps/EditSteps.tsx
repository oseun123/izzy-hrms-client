...
import { updateStep } from '../../../../../../store/actions/preferencesActions';
...
function EditSteps() {
  ...
  const { data, isLoading } = useGetSystemStep(enabled, setEnabled, 'all');

  // Effect to set selected designation when data changes
  useEffect(() => {
    if (data && Object.keys(data).length) {
      const steps = data.payload?.steps || [];
      const selected = steps.find((item) => item.id === parseInt(id));
  ...
  function editStepCallback() {
    setLoading(true);
    updateStep(dispatch, request, values).then((res) => {
      setLoading(false);
    });
  }
  ...
  const { values, errors, handleSubmit, clearForm, handleChange } =
    useCustomForm(editStepCallback, initValues, validateeditStep);
