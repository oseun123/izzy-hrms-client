...
function BranLogo({ currentCleint, setEnabled }) {
  const dispatch = useDispatch();
  const resquest = useAxiosPrivate();

  const init_settings = currentCleint?.settings;

  const [brand_variant, setBrandVarient] = useState(
    JSON.parse(init_settings)[0]?.display?.brand_variant || "navbar-light"
  );
  ...
  function onChangeNavbar(e) {
    ...
    const brand_all_colors = navbar_dark_skins.concat(navbar_light_skins);
    brand_all_colors.map((color) => brand_header.removeClass(color));

    brand_header.addClass(selected_color);

    setBrandVarient(selected_color);
  }
        ...
        <section className="content">
          ...
          <div className="card">
            <div className="card-header">
              <h3 className="card-title">Brand Logo Variants</h3>
            ...
            <div className="card-body">
              <div className="d-flex flex-wrap mb-3">
                <Radio.Group onChange={onChangeNavbar} value={brand_variant}>
                  <Space wrap>
